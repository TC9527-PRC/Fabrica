...
public:
    ...
    void set_qdot_map(std::map<std::string, VectorX> q_map);
    void set_state(const VectorX q, const VectorX qdot);
    
    // get state variables
    VectorX get_q();
    VectorX get_qdot();
    std::map<std::string, VectorX> get_q_map();
    std::map<std::string, VectorX> get_qdot_map();
    VectorX get_phi();
    ...
    bool check_u_completed(Vector2 pos_tol, Vector2 vel_tol, bool verbose = false);
    ...
    VectorX get_point_to_point_force(std::string name);
    VectorX get_ground_force(std::string name);
    VectorX get_SDF_force(std::string name);

    // gravity control
    void enable_gravity(std::string name);
    void disable_gravity(std::string name);
