...
void Robot::set_q_map(std::map<std::string, VectorX> q_map) {
    for (auto joint : _joints) {
        VectorX q = q_map[joint->_name];
        for (int i = 0;i < joint->_ndof;++i) {
            joint->_q[i] = q[i];
        }
    }
...
void Robot::set_qdot_map(std::map<std::string, VectorX> qdot_map) {
    for (auto joint : _joints) {
        VectorX qdot = qdot_map[joint->_name];
        for (int i = 0;i < joint->_ndof;++i) {
            joint->_qdot[i] = qdot[i];
        }
    }
}
...
std::map<std::string, VectorX> Robot::get_q_map() {
    std::map<std::string, VectorX> q_map;
    for (auto joint : _joints) {
        VectorX q = joint->_q;
        q_map[joint->_name] = q;
    }
...
std::map<std::string, VectorX> Robot::get_qdot_map() {
    std::map<std::string, VectorX> qdot_map;
    for (auto joint : _joints) {
        VectorX qdot = joint->_qdot;
        qdot_map[joint->_name] = qdot;
    }
    return qdot_map;
}
...
bool Robot::check_u_completed(dtype pos_tol, dtype vel_tol, bool verbose) {
    bool completed = true;
    for (auto actuator : _actuators) {
        if (dynamic_cast<ActuatorMotor*>(const_cast<Actuator*>(actuator)) != nullptr) {
            ActuatorMotor* actuator_ = dynamic_cast<ActuatorMotor*>(const_cast<Actuator*>(actuator));
            completed = completed && actuator_->check_completed(pos_tol, vel_tol, verbose);
        }
    }
    return completed;
}

bool Robot::check_u_completed(Vector2 pos_tol, Vector2 vel_tol, bool verbose) {
    bool completed = true;
    for (auto actuator : _actuators) {
        if (dynamic_cast<ActuatorMotor*>(const_cast<Actuator*>(actuator)) != nullptr) {
            ActuatorMotor* actuator_ = dynamic_cast<ActuatorMotor*>(const_cast<Actuator*>(actuator));
...
VectorX Robot::get_point_to_point_force(std::string name){
    VectorX FT = VectorX::Zero(6);
    for (auto force : _forces) {
        if (dynamic_cast<ForcePointToPoint*>(const_cast<Force*>(force)) != nullptr) {
            ForcePointToPoint* force_ = dynamic_cast<ForcePointToPoint*>(force);
            if (force_->_body0->_name == name) {
                Vector6 wrench = force_->getComputedForce().head(6);
                FT.head(3) = wrench.tail(3);
                FT.tail(3) = wrench.head(3);
                return FT;
            } else if (force_->_body1->_name == name) {
                Vector6 wrench = force_->getComputedForce().tail(6);
                FT.head(3) = wrench.tail(3);
                FT.tail(3) = wrench.head(3);
                return FT;
            }
...
VectorX Robot::get_ground_force(std::string name){
    VectorX FT = VectorX::Zero(6);
    for (auto force : _forces) {
        if (dynamic_cast<ForceGroundContact*>(const_cast<Force*>(force)) != nullptr) {
            ForceGroundContact* force_ = dynamic_cast<ForceGroundContact*>(force);
            if (force_->_contact_body->_name == name) {
                Vector6 wrench = force_->getComputedForce();
                FT.head(3) = wrench.tail(3);
                FT.tail(3) = wrench.head(3);
                return FT;
            }
...
VectorX Robot::get_SDF_force(std::string name){
    VectorX FT = VectorX::Zero(6);
    for (auto force : _forces) {
        if (dynamic_cast<ForceGeneralSDFContact*>(const_cast<Force*>(force)) != nullptr) {
            ForceGeneralSDFContact* force_ = dynamic_cast<ForceGeneralSDFContact*>(force);
            if (force_->_contact_body->_name == name) {
                Vector6 wrench = force_->getComputedForce().head(6);
                FT.head(3) = wrench.tail(3);
                FT.tail(3) = wrench.head(3);
                return FT;
            } else if (force_->_SDF_body->_name == name) {
                Vector6 wrench = force_->getComputedForce().tail(6);
                FT.head(3) = wrench.tail(3);
                FT.tail(3) = wrench.head(3);
                return FT;
...
// gravity control
void Robot::enable_gravity(std::string name) {
    for (auto body : _bodies) {
        if (body->_name == name) {
            body->enable_gravity();
        }
...
void Robot::disable_gravity(std::string name) {
    for (auto body : _bodies) {
        if (body->_name == name) {
