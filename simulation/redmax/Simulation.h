...
public:
    ...
    public:
        ...
        ViewerOptions() {
            ...
            _camera_pos = Vector3(1.25, -1.5, 1.5);
            _camera_up = Vector3::UnitZ();
            _camera_lookat = Vector3(-1, 1, 0);
            _ground = false;
    ...
    VectorX _q_init_fixed, _qdot_init_fixed;
    ...
    void set_joint_q_init(std::string joint_name, const VectorX q_init);
    void set_joint_qdot_init(std::string joint_name, const VectorX qdot_init);
    const VectorX get_q_init();
    const VectorX get_qdot_init();
    SE3 get_joint_init_transform(std::string joint_name);
    void set_joint_init_transform(std::string joint_name, const SE3 transform);
    void apply_joint_init_transform(std::string joint_name, const SE3 transform);
    ...
    void set_q_map(std::map<std::string, VectorX> q_map);
    void set_qdot_map(std::map<std::string, VectorX> qdot_map);
    const VectorX get_q();
    const VectorX get_qdot();
    const std::map<std::string, VectorX> get_q_map();
    const std::map<std::string, VectorX> get_qdot_map();
    void reparam();
    ...
    bool check_u_completed(dtype pos_tol, dtype vel_tol, bool verbose = false);
    bool check_u_completed_vec(Vector2 pos_tol, Vector2 vel_tol, bool verbose = false);
    ...
    VectorX get_joint_qm(const string name, bool full_dof = false);
    VectorX get_joint_qdot(const string name);
    VectorX get_joint_qmdot(const string name, bool full_dof = false);
    VectorX get_joint_q_from_qm(const string name, VectorX qm);
    VectorX get_joint_qm_from_q(const string name, VectorX q);
    ...
    SE3 get_joint_Ej0_init(const string name);

    // body related
    Body* get_body(const string name);
    dtype get_body_mass(const string name);
    Vector6 get_body_inertia(const string name);
    SE3 get_body_E0i(const string name);
    SE3 get_body_Ei0(const string name);
    SE3 get_body_E0j(const string name);
    std::map<std::string, SE3> get_body_E0j_map();
    VectorX get_body_qm(const string name, bool joint_rot = true);
    VectorX get_body_qm_from_joint_qm(const string name, VectorX joint_qm, bool joint_rot = true);
    void set_body_qm(const string name, VectorX qm, bool joint_rot = true);
    ...
    void enable_body_gravity(const string name);
    void disable_body_gravity(const string name);
    ...
    dtype get_body_distance_group(const std::vector<string> names_from, const std::vector<string> names_to);
    ...
    void set_body_color_map(std::map<std::string, Vector3> color_map);

    // VectorX get_Fm();
    VectorX get_acc();
    VectorX get_point_to_point_force(const string name);
    VectorX get_ground_force(const string name);
    VectorX get_SDF_force(const string name);
    ...
    // compute inverse dynamics for balance
    VectorX inverse_dynamics_balance();
    std::map<std::string, Vector6> inverse_dynamics_balance_map();
    ...
    // return matrices from simulation replay
    std::vector<std::map<std::string, SE3>> export_replay_matrices();
