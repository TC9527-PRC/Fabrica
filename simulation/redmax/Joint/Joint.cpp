...
SE3 Joint::get_init_transform() { // hack
    if (_parent != nullptr) {
        return math::Einv(_parent->_E_j0_0) * _E_pj_0;
    } else {
        return _E_pj_0;
    }
}

void Joint::set_init_transform(SE3 transform) { // hack
    // assume world frame
    if (_parent != nullptr) {
        _E_pj_0 = _parent->_E_j0_0 * transform;
    } else {
        _E_pj_0 = transform;
    }
    _E_jp_0 = math::Einv(_E_pj_0);
...
void Joint::apply_init_transform(SE3 transform) { // hack
    set_init_transform(transform * get_init_transform());
}
