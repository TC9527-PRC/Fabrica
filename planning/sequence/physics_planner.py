...
from assets.load import load_assembly, load_part_ids, load_config
from assets.transform import transform_pt_by_matrix
from planning.sequence.sim_string import get_path_sim_string, get_stability_sim_string, get_contact_sim_string
...
class MultiPartPathPlanner:
    ...
    def __init__(self, asset_folder, assembly_dir, parts_fix, part_move, parts_removed=[], pose=None, force_mag=FORCE_MAG, save_sdf=False, adaptive_sample=False,
        camera_pos=None, camera_lookat=None):
        model_string = get_contact_sim_string(assembly_dir, parts_fix + [part_move] + parts_removed, save_sdf=save_sdf, adaptive_sample=adaptive_sample)
        ...
        model_string = get_path_sim_string(assembly_dir, parts_fix, part_move, parts_removed=parts_removed, 
            save_sdf=save_sdf, pose=pose, col_th=col_th_dict, adaptive_sample=adaptive_sample)
        ...
        if config is not None:
            if 'contact_eps' in config: self.contact_eps = config['contact_eps']
            if 'col_th_path' in config: self.col_th_base = config['col_th_path']
            if 'min_sep' in config: self.min_sep = config['min_sep']
    ...
    def check_success(self, action, return_path=False, min_sep=None, max_path_len=None):
        ...
        while True:
            ...
            for _ in range(self.frame_skip):
                ...
                t_plan = time() - t_start
                if t_sim > self.max_sim_time or t_plan > self.max_plan_time or (max_path_len is not None and len(path) > max_path_len):
                    if return_path:
                        return False, path
                    else:
                        return False
...
class StabilityChecker:
    ...
    def __init__(self, allow_gap, contact_eps=None):
        self.sim = None
...
class MultiPartStabilityPlanner:
    ...
    def __init__(self, asset_folder, assembly_dir, parts_fix, parts_move, pose=None, save_sdf=False, allow_gap=False):
        ...
        config = load_config(assembly_dir)
        if config is not None:
            if 'col_th_stable' in config: self.col_th = config['col_th_stable']
            if 'contact_eps' in config: self.contact_eps = config['contact_eps']
    ...
    def check_success(self, max_step=MAX_STEP, timeout=None):
        ...
        self.sim.reset()
        checker = StabilityChecker(self.allow_gap, self.contact_eps)
...
def get_contact_graph(asset_folder, assembly_dir, parts=None, contact_eps=None, ignore_config=False, save_sdf=False):
    '''
    Get contact graph for assembly
    '''
    if contact_eps is None: contact_eps = CONTACT_EPS
    config = load_config(assembly_dir)
    if config is not None and 'contact_eps' in config and not ignore_config: contact_eps = config['contact_eps']
