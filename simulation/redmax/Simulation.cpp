...
void Simulation::init(bool verbose) {
    ...
    _q_init = get_q();
    _qdot_init = get_qdot();
    _q_init_fixed = _q_init;
    _qdot_init_fixed = _qdot_init;
}
...
void Simulation::set_joint_q_init(std::string joint_name, const VectorX q_init) {
    Joint* joint = get_joint(joint_name);
    int ndof = joint->_ndof;
    if (q_init.size() != ndof) {
        std::cerr << "[Error] set_joint_q_init: q_init.size() != ndof." << std::endl;
        throw "error";
    }
    _q_init.segment(joint->_index[0], ndof) = q_init;
}
...
void Simulation::set_joint_qdot_init(std::string joint_name, const VectorX qdot_init) {
    Joint* joint = get_joint(joint_name);
    int ndof = joint->_ndof;
    if (qdot_init.size() != ndof) {
        std::cerr << "[Error] set_joint_qdot_init: qdot_init.size() != ndof." << std::endl;
        throw "error";
    }
    _qdot_init.segment(joint->_index[0], ndof) = qdot_init;
}
...
SE3 Simulation::get_joint_init_transform(std::string joint_name) {
    Joint* joint = get_joint(joint_name);
    return joint->get_init_transform();
}

void Simulation::set_joint_init_transform(std::string joint_name, const SE3 transform) {
    Joint* joint = get_joint(joint_name);
    joint->set_init_transform(transform);
}

void Simulation::apply_joint_init_transform(std::string joint_name, const SE3 transform) {
    Joint* joint = get_joint(joint_name);
    joint->apply_init_transform(transform);
}

// states
void Simulation::reset_state() {
    set_q(_q_init_fixed);
    set_qdot(_qdot_init_fixed);
}
...
void Simulation::set_q_map(std::map<std::string, VectorX> q_map) {
    _robot->set_q_map(q_map);
}

void Simulation::set_qdot_map(std::map<std::string, VectorX> qdot_map) {
    _robot->set_qdot_map(qdot_map);
}
...
const std::map<std::string, VectorX> Simulation::get_q_map() {
    return _robot->get_q_map();
}

const std::map<std::string, VectorX> Simulation::get_qdot_map() {
    return _robot->get_qdot_map();
}
...
bool Simulation::check_u_completed(dtype pos_tol, dtype vel_tol, bool verbose) {
    return _robot->check_u_completed(pos_tol, vel_tol, verbose);
}

bool Simulation::check_u_completed_vec(Vector2 pos_tol, Vector2 vel_tol, bool verbose) {
    return _robot->check_u_completed(pos_tol, vel_tol, verbose);
}
...
VectorX Simulation::get_joint_qm(const string name, bool full_dof) {
    Joint* joint = get_joint(name);
    if (full_dof || joint->_ndof == 6) {
...
VectorX Simulation::get_joint_qmdot(const string name, bool full_dof) {
    Joint* joint = get_joint(name);
    if (full_dof || joint->_ndof == 6) { // translation + rotation
        Vector6 qmdot = Vector6::Zero();
        qmdot.head(3) = joint->_phi.tail(3); // _phi: rot, trans
        qmdot.tail(3) = joint->_phi.head(3); // qmdot: trans, rot
        return qmdot;
    } else if (joint->_ndof == 3) { // translation only
...
VectorX Simulation::get_joint_q_from_qm(const string name, VectorX qm) {
    Joint* joint = get_joint(name);
    if (joint->_ndof != qm.size()) throw_error("get_joint_q_from_qm: joint ndof mismatch");
    ...
    if (joint->_ndof == 3) {
        return Q.topRightCorner(3, 1);
    } else if (joint->_ndof == 6) {
        Vector6 q = Vector6::Zero();
        q.head(3) = Q.topRightCorner(3, 1);
        q.tail(3) = math::log(Q.topLeftCorner(3, 3));
        return q;
    } else {
        throw_error("get_joint_q_from_qm: joint ndof not supported: " + std::to_string(joint->_ndof));
    }
}

VectorX Simulation::get_joint_qm_from_q(const string name, VectorX q) {
    Joint* joint = get_joint(name);
    if (joint->_ndof != q.size()) throw_error("get_joint_qm_from_q: joint ndof mismatch");

    Matrix4 Q = Matrix4::Identity();
    Q.topRightCorner(3, 1) = q.head(3);
    Vector3 q_tail = q.tail(3);
    Q.topLeftCorner(3, 3) = math::exp(q_tail);
    Matrix4 E_0j = joint->_E_pj_0 * Q;

    if (joint->_ndof == 3) {
        return E_0j.topRightCorner(3, 1);
    } else if (joint->_ndof == 6) {
        Vector6 qm = Vector6::Zero();
        qm.head(3) = E_0j.topRightCorner(3, 1);
        qm.tail(3) = math::mat2euler(E_0j.topLeftCorner(3, 3));
        return qm;
    } else {
        throw_error("get_joint_qm_from_q: joint ndof not supported: " + std::to_string(joint->_ndof));
    }
}
...
SE3 Simulation::get_joint_Ej0_init(const string name) {
    Joint* joint = get_joint(name);
    return joint->_E_j0_0;
}
...
Vector6 Simulation::get_body_inertia(const string name) {
    return get_body(name)->_Inertia;
}
...
SE3 Simulation::get_body_E0j(const string name) {
    Body* body = get_body(name);
    return body->_joint->_E_0j;
}

// void Simulation::set_body_E0j(const string name, SE3 E0j) {
//     Body* body = get_body(name);
//     body->_joint->_E_0j = E0j;
// }

std::map<std::string, SE3> Simulation::get_body_E0j_map() {
    std::map<std::string, SE3> E0j_map;
    for (auto iter = _body_map.begin(); iter != _body_map.end(); iter++) {
        std::string name = iter->first;
        Body* body = iter->second;
        E0j_map[name] = body->_joint->_E_0j;
    }
    return E0j_map;
}

VectorX Simulation::get_body_qm(const string name, bool joint_rot) {
    Body* body = get_body(name);
    Vector6 qm = Vector6::Zero();
    Matrix3 R2;
    if (joint_rot) R2 = body->_joint->_E_0j.topLeftCorner(3, 3);
    else R2 = body->_E_0i.topLeftCorner(3, 3);
...
VectorX Simulation::get_body_qm_from_joint_qm(const string name, VectorX joint_qm, bool joint_rot) {
    Body* body = get_body(name);
    Matrix4 joint_E_0j = Matrix4::Identity();
    joint_E_0j.topLeftCorner(3, 3) = math::euler2mat(joint_qm.tail(3));
    joint_E_0j.topRightCorner(3, 1) = joint_qm.head(3);
    Matrix4 body_E_0i = joint_E_0j * body->_E_ji;
    if (joint_rot) body_E_0i.topLeftCorner(3, 3) = joint_E_0j.topLeftCorner(3, 3);
    Vector6 body_qm = Vector6::Zero();
    body_qm.head(3) = body_E_0i.topRightCorner(3, 1);
    body_qm.tail(3) = math::mat2euler(body_E_0i.topLeftCorner(3, 3));
    return body_qm;
}

void Simulation::set_body_qm(const string name, VectorX qm, bool joint_rot) {
    Body* body = get_body(name);
    Matrix4 body_E_0i = Matrix4::Identity();
    if (joint_rot) body_E_0i.topLeftCorner(3, 3) = math::euler2mat(qm.tail(3)) * body->_E_ji.topLeftCorner(3, 3);
    else body_E_0i.topLeftCorner(3, 3) = math::euler2mat(qm.tail(3));
    body_E_0i.topRightCorner(3, 1) = qm.head(3);
    Joint* joint = body->_joint;
    Matrix4 joint_E_0j = body_E_0i * body->_E_ij;
    Matrix4 joint_E_j0 = math::Einv(joint_E_0j);
    if (joint->_ndof == 6) { // update joint Q
        Vector6 joint_qm = Vector6::Zero();
        joint_qm.head(3) = joint_E_0j.topRightCorner(3, 1);
        joint_qm.tail(3) = math::mat2euler(joint_E_0j.topLeftCorner(3, 3));
        set_joint_qm(joint->_name, joint_qm);
    }
    else { // update joint E_pj_0
        if (joint->_parent == nullptr) {
            joint->_E_jp_0 = joint->_Q * joint_E_j0;
            joint->_E_j0_0 = joint->_E_jp_0;
        } else {
            joint->_E_jp_0 = joint->_Q * joint_E_j0 * joint->_parent->_E_0j;
            joint->_E_j0_0 = joint->_E_jp_0 * joint->_parent->_E_j0_0;
        }
        joint->_E_pj_0 = math::Einv(joint->_E_jp_0);
    }
}
...
void Simulation::enable_body_gravity(const string name) {
    get_body(name)->enable_gravity();
}

void Simulation::disable_body_gravity(const string name) {
    get_body(name)->disable_gravity();
}
...
dtype Simulation::get_body_distance(const string name_from, const string name_to) {
    ...
    } else if (dynamic_cast<BodyMultiSDFObj*>(const_cast<Body*>(body_to)) != nullptr) {
        BodyMultiSDFObj* body_to_ = dynamic_cast<BodyMultiSDFObj*>(const_cast<Body*>(body_to));
        for (int i = 0; i < d.size(); ++i) {
            d(i) = body_to_->distance(vertices_from.col(i));
        }
...
dtype Simulation::get_body_distance_group(const std::vector<string> names_from, const std::vector<string> names_to) {
    Matrix3X vertices_from;
    for (int i = 0; i < names_from.size(); ++i) {
        Matrix3X vertices_i_from = get_body_vertices(names_from[i], true);
        vertices_from.conservativeResize(3, vertices_from.cols() + vertices_i_from.cols());
        vertices_from.rightCols(vertices_i_from.cols()) = vertices_i_from;
    }
    dtype d_min = 1e10;
    for (int i = 0; i < names_to.size(); ++i) {
        Body* body_to = get_body(names_to[i]);
        VectorX d;
        d.resize(vertices_from.cols(), 1);
        if (dynamic_cast<BodyPrimitiveShape*>(const_cast<Body*>(body_to)) != nullptr) {
            BodyPrimitiveShape* body_to_ = dynamic_cast<BodyPrimitiveShape*>(const_cast<Body*>(body_to));
        ...
        d_min = std::min(d.minCoeff(), d_min);
    }
    return d_min;
}
...
void Simulation::set_body_color_map(std::map<std::string, Vector3> color_map) {
    for (auto iter = color_map.begin(); iter != color_map.end(); iter++) {
        Body* body = _body_map[iter->first];
        body->set_color(iter->second);
    }
}

VectorX Simulation::get_acc() {
    ...
    VectorX Fm = J * (JT * (fm - MmJdot * qdot) + fr);
    VectorX acc = Fm.array() / Mm.array();
    
    return acc;
}

VectorX Simulation::get_point_to_point_force(const string name) {
    return _robot->get_point_to_point_force(name);
}

VectorX Simulation::get_ground_force(const string name) {
    return _robot->get_ground_force(name);
}

VectorX Simulation::get_SDF_force(const string name) {
    return _robot->get_SDF_force(name);
}
...
void Simulation::newton(VectorX& x,
    ...
    for (int iter_newton = 0;iter_newton < MaxIter_Newton;iter_newton ++) {
        ...
        if (ls_trial < 0) ls_trial = MaxIter_LS;

        // std::cerr << "[iter " << iter_newton << ", LS " << ls_trial << "] g_new = " << g_new.norm() << ", g = " << gnorm << std::endl;
...
VectorX Simulation::inverse_dynamics_balance() {
    if (_options->_integrator == "BDF1") {
        VectorX q0 = get_q(), qdot0 = get_qdot();
        VectorX q1 = q0;
        // VectorX q1 = q0 + _h * qdot0;
        return inverse_dynamics_BDF1(q0, qdot0, q1, true);
    } else {
        std::cerr << "[Error] Inverse dynamics balance of integrator " << _options->_integrator << " has not been implemented." << std::endl;
        throw "error";
    }
}

std::map<std::string, Vector6> Simulation::inverse_dynamics_balance_map() {
    VectorX f_external = inverse_dynamics_balance();
    std::map<std::string, Vector6> f_external_map;
    for (auto body : _robot->_bodies) {
        Vector6 f_body_external = f_external.segment(body->_index[0], 6);
        f_external_map[body->_name] = f_body_external;
        // f_external_map[body->_name] = f_body_external.array() / body->_Inertia.array();
    }
    return f_external_map;
}
...
std::vector<std::map<std::string, SE3>> Simulation::export_replay_matrices() {
    std::vector<std::map<std::string, SE3>> matrices;
    for (int i = 0;i < _q_his.size();i++) {
        set_state(_q_his[i], _qdot_his[i]);
        update_robot();
        std::map<std::string, SE3> matrix_map;
        for (auto body : _robot->_bodies) {

            SE3 scale_matrix = SE3::Identity();
            scale_matrix.block<3, 3>(0, 0) = body->_scale.asDiagonal();

            if (dynamic_cast<BodyAbstract*>(body) != nullptr) {
                BodyAbstract* body_ = dynamic_cast<BodyAbstract*>(body);
                matrix_map[body->_name] = body_->_E_0i * body_->_visual_frame * scale_matrix;
            } else if (dynamic_cast<BodyMeshObj*>(body) != nullptr) {
                BodyMeshObj* body_ = dynamic_cast<BodyMeshObj*>(body);
                matrix_map[body->_name] = body_->_E_0i * body_->_E_io * scale_matrix;
            } else {
                Body* body_ = dynamic_cast<Body*>(body);
                matrix_map[body->_name] = body_->_E_0i * scale_matrix;
            }
        }
        matrices.push_back(matrix_map);
    }
    return matrices;
}
